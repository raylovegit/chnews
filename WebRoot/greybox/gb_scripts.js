...
GreyBox = new AJS.Class({init:function (c) {
	this.use_fx = AJS.fx;
	this.type = "page";
	...
	setTimeout(function () {
		...
		if (b.use_fx) {
			var e = b.overlay;
			AJS.fx.fadeOut(b.overlay, {onComplete:function () {
				AJS.removeElement(e);
				e = null;
			}, duration:300});
...
GB_Gallery = GreyBox.extend({init:function (a) {
	this.parent({});
...
GB_Sets = GB_Gallery.extend({init:function (a, b) {
	this.parent(a);
	if (!this.img_next) {
...
GB_Window = GreyBox.extend({init:function (a) {
	this.parent({});
